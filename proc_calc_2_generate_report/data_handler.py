...
def proc_summary_detail(
    ...
    for item in filter_list_sale:
        if now_series_code != item[18]:
            ...
            if now_series_count > 0:
                final_list.append(
                    [now_series_title, now_series_sales_sum, now_series_calc_sum]
                )
                # init
                now_series_count = 0
                now_series_sales_sum = 0
    ...
    if now_series_count > 0:
        final_list.append([now_series_title, now_series_sales_sum, now_series_calc_sum])
    ...
    if len(final_list) > 0:
        final_list.append([str_sum, sum_sales_sum, sum_calc_sum])
    ...
    if cp_deduction_type == "후차감":  # 4개
        ...
        detail_headers = locale_contants["summary_2_detail_4_headers"]
        detail_list = []
        for item in final_list:
            detail_list.append([item[0], item[1], item[2], "", item[2]])
        ...
        df_summary_detail = pd.DataFrame(detail_list, columns=detail_headers)
        # df_summary_detail.style.set_caption("Hello World 2")
        # df_summary_detail = df_summary_detail.style.set_caption("Hello World 2")

    # df_summary_detail = df_summary_detail.reset_index(drop=True, inplace=True)
    # df_summary_detail = df_summary_detail.style.set_table_attributes(
...
def proc_summary_total(
    ...
    print("start - proc_summary_total()")
    ...
    # df_detail = df_summary_detail.get("list")
    ...
    if cp_tax_type == "면세":
        ...
        if cp_biz_type == "개인":
            amount_content = df_summary_detail[detail_row - 1][detail_col - 1]
        else:
            # 사업자인 경우, 합계 > 세액 > 공급가액 계산
            total_calc = df_summary_detail[detail_row - 1][detail_col - 1]
            amount_content = df_summary_detail[detail_row - 1][detail_col - 1]
